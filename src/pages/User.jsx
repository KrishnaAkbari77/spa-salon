    ...
    const fetchData = async () => {
      try {
        const apptRes = await fetch(
          `http://localhost:3001/appointments?userId=${user.id}`,
        );
        ...
        const msgRes = await fetch(
          `http://localhost:3001/messages?userId=${user.id}`,
        );
        ...
        const fbRes = await fetch(
          `http://localhost:3001/feedbacks?userId=${user.id}`,
        );
    ...
    <div className={`appointment-card ${isPast ? "past" : ""}`}>
      ...
      <div className="appt-details">
        ...
        <div className="detail-row">
          <MapPin size={16} />{" "}
          <span>
            {app.place} {app.address ? `- ${app.address}` : ""}
          </span>
        </div>
        <div className="detail-row">
          <CreditCard size={16} />{" "}
          <span>
            {app.price} •{" "}
            {app.paymentMethod === "razorpay" ? "Razorpay" : "Cash on Delivery"}
          </span>
        </div>
      </div>
    </div>
        ...
        <div className="notifications-section mt-40">
          ...
          >
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`notification-card card ${msg.read ? "read" : "unread"}`}
                style={{
                  padding: "20px",
              ...
              >
                ...
                >
                  <span
                    ...
                    style={{ fontSize: "12px", color: "var(--light-text)" }}
                  >
